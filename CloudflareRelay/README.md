# Patchwork Cloudflare relay

Cloudflare Worker and SQLite-backed Durable Object for Patchwork's hosted, end-to-end encrypted remote relay. The Worker routes `/relay/*` plus mutable `/js/*` and `/css/*` assets before the static Pi Desk site and serves other requests from `../Sources/PatchworkWeb/Site`.
