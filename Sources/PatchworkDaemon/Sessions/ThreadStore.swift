...
actor ThreadStore {
    ...
    private var presented: [PatchworkThread] = []
    private var hasPresentedCatalog = false
    private var presentedByPath: [String: PatchworkThread] = [:]
    private var presentedByID: [String: [PatchworkThread]] = [:]
    ...
    init(
        ...
        if let indexFileURL,
           ...
           let persisted = try? PatchworkJSON.decoder.decode(PersistedIndex.self, from: data),
    ...
    func listThreads(
        ...
        sidebar: Bool = false
    ) async throws -> (threads: [PatchworkThread], nextCursor: String?) {
        pruneListSnapshots()
        var legacyOffset = 0
        ...
        for index in filtered.indices {
            filtered[index].shortId = PatchworkThread.abbreviatedID(for: filtered[index].id)
    ...
    private func page(
        _ threads: [PatchworkThread], start: Int, limit: Int, snapshotToken: String
    ) -> (threads: [PatchworkThread], nextCursor: String?) {
    ...
    func thread(idOrPath: String) async -> PatchworkThread? {
    ...
    func presentCreatedThread(
        _ thread: PatchworkThread, runningOverride: Bool? = nil
    ) async -> PatchworkThread {
        ...
        presentedThread.shortId = PatchworkThread.abbreviatedID(for: presentedThread.id)
    ...
    private func adoptExistingThread(at url: URL) async -> PatchworkThread? {
    ...
    private func cacheThread(_ thread: PatchworkThread, at url: URL) {
    ...
    private func presentIfCatalogLoaded(_ thread: PatchworkThread) {
    ...
    /// callers use the compact UUID tail printed by `patchwork` without risking the wrong thread.
    func resolve(idOrPath: String) async throws -> PatchworkThread? {
        let standardizedPath = URL(fileURLWithPath: idOrPath).standardizedFileURL.path
    ...
    func resolveForMutation(idOrPath: String) async throws -> PatchworkThread? {
        let standardizedPath = URL(fileURLWithPath: idOrPath).standardizedFileURL.path
        if let exactPath = presentedByPath[standardizedPath] {
            return await thread(idOrPath: exactPath.path)
        }
    ...
    private func resolve(idOrPath: String, among threads: [PatchworkThread]) throws -> PatchworkThread? {
        let standardizedPath = URL(fileURLWithPath: idOrPath).standardizedFileURL.path
    ...
    func refreshedThread(idOrPath: String) async -> PatchworkThread? {
        let standardizedPath = URL(fileURLWithPath: idOrPath).standardizedFileURL.path
    ...
    func activityProjection(
        ...
        threadsWithoutHeartbeat: [PatchworkThread], unreadCount: Int,
        ...
        if hasPresentedCatalog, await catalogIsCurrent(supplementalPaths: supplementalPaths) {
            ...
            // transcript, but avoid rebuilding every PatchworkThread and every point-lookup index merely
    ...
    @discardableResult
    func setArchived(_ archived: Bool, idOrPath: String) async throws -> PatchworkThread {
    ...
    @discardableResult
    func setUnread(_ unread: Bool, idOrPath: String) async throws -> PatchworkThread {
    ...
    private nonisolated static func scanCatalog(
        ...
        var orderedResults = Array<PatchworkThread?>(repeating: nil, count: catalog.sessions.count)
    ...
    private func publishPresented(_ threads: [PatchworkThread]) {
    ...
    private static func parse(_ url: URL, agent: AgentKind, logger: DaemonLogger) -> PatchworkThread? {
        do {
