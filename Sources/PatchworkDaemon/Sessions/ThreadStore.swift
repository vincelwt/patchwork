...
actor ThreadStore {
    ...
    private struct CacheEntry { let fingerprint: Fingerprint; let thread: PatchworkThread? }
    ...
    ) async -> (threads: [PatchworkThread], nextCursor: String?) {
    ...
    func threadsWithoutHeartbeat(excluding heartbeatIDs: Set<String>) async -> [PatchworkThread] {
    ...
    private func allThreadsSorted() async -> [PatchworkThread] {
        let files = SessionScanner.discoverSessions(roots: roots)
        var results: [PatchworkThread] = []
