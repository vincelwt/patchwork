...
/// The complete Patchwork control plane without a process policy. Patchwork hosts one of these
/// directly; the optional `patchworkd` executable is only another host for the same service.
public final class PatchworkControlService: @unchecked Sendable {
    ...
    public convenience init() {
        ...
        self.init(
            ...
            core: core,
            unixSocketPath: PatchworkPaths.controlSocket
        )
    }
    ...
    public func start() async throws {
        ...
        // terminate, whether the host is Patchwork or the optional standalone executable.
        signal(SIGPIPE, SIG_IGN)
        logger.info("Patchwork control service starting (concurrency=\(settings.concurrency), remoteEnabled=\(settings.remoteEnabled), port=\(settings.port))")
        connectivity?.start()
        ...
        do {
            ...
            logger.info("Patchwork control service ready")
        } catch {
            server.stop()
            if coreStarted { await core.stop(graceSeconds: 0) }
            connectivity?.stop()
            logger.error("Could not start Patchwork control service: \(error)")
            logger.flush()
            markStopped()
            throw error
        }
    ...
    public func stop(graceSeconds: TimeInterval = 10) async {
        ...
        logger.info("Patchwork control service shutting down")
        server.stop()
