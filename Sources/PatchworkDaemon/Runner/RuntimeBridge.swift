...
final class InteractionRegistry: @unchecked Sendable {
    ...
    private let timerQueue = DispatchQueue(label: "app.patchwork.desktop.daemon.interactions")
...
actor SubmissionRegistry {
    ...
    init(fileURL: URL? = nil, logger: DaemonLogger? = nil) {
        ...
        guard size > 0, size <= Self.maximumPersistedBytes,
              ...
              let envelope = try? PatchworkJSON.decoder.decode(PersistedEnvelope.self, from: data),
    ...
    private func persist() -> Bool {
        ...
        do {
            ...
            let data = try PatchworkJSON.encoder.encode(envelope)
