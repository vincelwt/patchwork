...
actor RunHistoryStore {
    ...
    func reconcileAfterHostRestart(now: Date = Date()) {
        for index in recent.indices where recent[index].trigger != .schedule
            ...
            run.error = run.promptStartedAt == nil
                ? "Patchwork closed before this run started."
    ...
    private static func load(fileURL: URL, maxInMemory: Int, logger: DaemonLogger) -> [Run] {
        ...
        func ingest(_ record: Data) {
            guard let run = try? PatchworkJSON.decoder.decode(Run.self, from: record) else {
    ...
    private func rotateIfNeeded() {
        ...
        var rewritten = Data()
        for run in recent {
            guard var line = try? PatchworkJSON.encoder.encode(run) else { continue }
            line.append(0x0A)
