...
private struct StoredRelayIdentity: Codable {
    ...
    static func loadOrCreate(at url: URL) throws -> StoredRelayIdentity {
        if var stored = PatchworkFile.readIfPresent(StoredRelayIdentity.self, from: url),
           ...
           !stored.hostToken.isEmpty {
            if stored.protocolVersion != relayProtocolVersion {
                ...
                try PatchworkFile.writeAtomic(stored, to: url)
            }
            return stored
        }
        ...
        try PatchworkFile.writeAtomic(identity, to: url)
        return identity
    }
...
actor RelayService {
    ...
    init(
        identityFileURL: URL = PatchworkPaths.relayIdentity,
    ...
    private func saveIdentity(_ value: StoredRelayIdentity) throws {
        try PatchworkFile.writeAtomic(value, to: identityFileURL)
        identity = value
    }
    ...
    private func connectAndReceive() async throws {
        ...
        while !Task.isCancelled {
            ...
            let message = try PatchworkJSON.decoder.decode(RelayWireMessage.self, from: data)
    ...
    private func send(_ message: RelayWireMessage) async throws {
        ...
        let data = try PatchworkJSON.encoder.encode(message)
        guard data.count <= Self.maxRPCBodyBytes else { throw URLError(.dataLengthExceedsMaximum) }
    ...
    private func handleRPC(deviceID: String, connectionID: String?, payload: RelayCipherPayload) async {
        ...
        do {
            ...
            let rpc = try PatchworkJSON.decoder.decode(RemoteRPCRequest.self, from: plaintext)
            ...
            var responseData = try PatchworkJSON.encoder.encode(response)
            if responseData.count > Self.maxEncryptedPlaintextBytes {
                responseData = try PatchworkJSON.encoder.encode(
    ...
    private func broadcastEvent(
        ...
        guard let plaintext = try? PatchworkJSON.encoder.encode(event), plaintext.count <= Self.maxEncryptedPlaintextBytes else { return }
