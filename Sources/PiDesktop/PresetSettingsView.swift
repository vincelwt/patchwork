...
struct PresetSettingsView: View {
            ...
            VStack(alignment: .leading, spacing: PiTheme.space16) {
                HStack {
                    VStack(alignment: .leading, spacing: PiTheme.space4) {
                        Text("Presets").font(PiFont.title)
    ...
    private func presetRow(_ preset: AgentPreset, at index: Int) -> some View {
        HStack(spacing: PiTheme.space10) {
            AgentBadge(agent: preset.agent, size: PiTheme.agentBadgeSize + 2)
            VStack(alignment: .leading, spacing: PiTheme.space2) {
                Text(preset.name).font(PiFont.rowEmphasis)
                Text(preset.detail)
                    .font(PiFont.micro)
            ...
            Menu {
                ...
                Image(systemName: "ellipsis")
                    .frame(width: PiTheme.space20, height: PiTheme.space20)
            }
    ...
    private func editor(_ value: AgentPresetDraft) -> some View {
        VStack(alignment: .leading, spacing: PiTheme.space12) {
            ...
            if let error = loader.error {
                Text(error).font(PiFont.micro).foregroundStyle(Color.piRed)
            }
