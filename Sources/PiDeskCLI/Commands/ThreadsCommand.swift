...
enum ThreadsCommand {
    ...
    private static let newHelp = CommandHelp(
        usage: "pidesk threads new --cwd DIR [--agent AGENT] [--worktree] [--name NAME] [--message TEXT] [--mode MODE] [--client-id ID] [--json]",
        ...
        examples: [
            ...
            "pidesk threads new --cwd ~/code/myapp --agent claude --message \"survey the repo\"",
            "pidesk threads new --cwd ~/code/myapp --client-id nightly_triage_1"
        ]
    )
    ...
    private static func new(_ args: [String], context: CommandContext) async -> Int32 {
        await runLeaf(args, context: context, help: newHelp) { parsed, global in
            ...
            if let error = response.firstMessageError {
                ...
                context.out.errorLine("Retry it with `pidesk threads send \(id) -` and provide the message on stdin.")
            }
        }
    }
    ...
    private static let sendHelp = CommandHelp(
        usage: "pidesk threads send <id> <text|-> [--steer | --follow-up] [--wait] [--client-id ID] [--json]",
        ...
        examples: [
            ...
            "pidesk threads send 019f9dea-... \"continue\" --client-id followup_1"
        ]
    )
