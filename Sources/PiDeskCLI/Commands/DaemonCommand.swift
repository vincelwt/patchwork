...
enum DaemonCommand {
    static let groupHelp = GroupHelp(
        name: "daemon",
        usage: "pidesk daemon <subcommand> [args]",
        summary: "Install, start, stop, and inspect pi-deskd, the control-plane daemon.",
        subcommands: [
            ("status", "Show whether the daemon is reachable and its health"),
            ("start", "Start the daemon"),
            ("stop", "Stop the daemon"),
            ("restart", "Restart the daemon"),
            ("install", "Register pi-deskd as a login item (LaunchAgent)"),
    ...
    private static let startHelp = CommandHelp(usage: "pidesk daemon start", summary: "Start pi-deskd (via launchctl if installed, otherwise a direct spawn).", flags: [], examples: ["pidesk daemon start"])
    private static let stopHelp = CommandHelp(usage: "pidesk daemon stop", summary: "Stop pi-deskd via launchctl.", flags: [], examples: ["pidesk daemon stop"])
    private static let restartHelp = CommandHelp(usage: "pidesk daemon restart", summary: "Restart pi-deskd via launchctl. Use after `pidesk remote enable/disable`.", flags: [], examples: ["pidesk daemon restart"])
    ...
    private static let installHelp = CommandHelp(usage: "pidesk daemon install", summary: "Register pi-deskd as a LaunchAgent that starts at login and restarts on failure.", flags: [], examples: ["pidesk daemon install"])
    ...
    private static let uninstallHelp = CommandHelp(usage: "pidesk daemon uninstall", summary: "Remove the LaunchAgent registration.", flags: [], examples: ["pidesk daemon uninstall"])
    ...
    private static let logsHelp = CommandHelp(
        usage: "pidesk daemon logs [-f] [--lines N] [--json]",
        ...
        examples: ["pidesk daemon logs", "pidesk daemon logs -f", "pidesk daemon logs --lines 500 --json"]
    )
    ...
    private static func logs(_ args: [String], context: CommandContext) async -> Int32 {
        await runLeaf(args, context: context, help: logsHelp) { parsed, global in
            ...
            guard context.fileManager.fileExists(atPath: path.path) else {
                throw CLIFailure(exitCode: .requestFailed, message: "no log file at \(path.path) yet", hint: "the daemon hasn't run; try `pidesk daemon start` first")
            }
