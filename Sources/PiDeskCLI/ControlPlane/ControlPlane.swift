...
/// bottom of this file for exactly what changes when `PiDeskKit.PiDeskClient` lands.
...
/*
 Adopting PiDeskKit.PiDeskClient once it lands:

 1. Delete Models/WireModels.swift and re-point every `Wire*` type alias at PiDeskKit's
 ...
 2. Delete RawHTTPClient.swift/RawSocketTransport.swift and replace HTTPControlPlane's guts with
    calls into PiDeskKit.PiDeskClient, keeping this same `ControlPlane` protocol and the same
    method signatures so Commands/ and Tests/ don't change at all.
 3. Drop the transport-only tests (RawHTTPClientTests) — PiDeskKit owns that layer's coverage now.
