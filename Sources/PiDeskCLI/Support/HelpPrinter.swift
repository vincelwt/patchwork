...
/// A leaf command's `--help` content (e.g. `pidesk threads list --help`).
struct CommandHelp {
    var usage: String
    var summary: String
    var flags: [FlagSpec]
...
/// A group's `--help` content (e.g. `pidesk threads --help`), listing its subcommands.
struct GroupHelp {
    var name: String
    var usage: String
    var summary: String
...
enum HelpPrinter {
    ...
    static func render(_ help: GroupHelp) -> String {
        ...
        lines.append("")
        lines.append("Run `pidesk \(help.name) <subcommand> --help` for details on a subcommand.")
        return lines.joined(separator: "\n")
    }
