...
enum GlobalFlag {
    static let socket = FlagSpec("--socket", takesValue: true, placeholder: "PATH", help: "control socket path (default: ~/Library/Application Support/Pi Desktop/daemon.sock)")
    ...
    static let token = FlagSpec("--token", takesValue: true, placeholder: "TOKEN", help: "bearer token for --url (default: $PIDESK_TOKEN)")
    static let timeout = FlagSpec("--timeout", takesValue: true, placeholder: "SECONDS", help: "per-request timeout in seconds (default: 10, or $PIDESK_TIMEOUT)")
    ...
    /// semantics; the global request timeout then falls back to $PIDESK_TIMEOUT or the default.
...
struct GlobalOptions {
    ...
    static func resolve(_ parsed: ParsedArgs, environment: [String: String], commandOwnsTimeout: Bool) throws -> GlobalOptions {
        let socketPath = parsed.value("--socket") ?? PiDeskPaths.controlSocket.path
