            ...
            if store.isScanning, snapshot.activeGroups.isEmpty, snapshot.archivedSessions.isEmpty {
                ...
                if mode == .status {
                    ...
                    ScrollView {
                        ...
                        .padding(.horizontal, PatchworkTheme.space6)
                        .padding(.bottom, PatchworkTheme.space12)
                    }
...
private struct SidebarActionRow: View {
    ...
    var body: some View {
        Button(action: action) {
            ...
            .frame(height: PatchworkTheme.sidebarRowHeight)
            .contentShape(Rectangle())
            .patchworkRowBackground(selected: selected, hovering: hovering)
        }
        .buttonStyle(.plain)
        .onHover { hovering = $0 }
        .padding(.horizontal, PatchworkTheme.space6)
...
private struct ForeignConversationsRow: View {
    ...
    var body: some View {
        Button { store.setShowsForeignConversations(true) } label: {
            ...
            .padding(.horizontal, PatchworkTheme.sidebarIconInset)
            .frame(height: PatchworkTheme.sidebarRowHeight)
            .contentShape(Rectangle())
        }
        .buttonStyle(.plain)
...
private struct SidebarFooter: View {
    ...
    var body: some View {
        HStack(spacing: PatchworkTheme.space6) {
            StatusDot(color: runningCount > 0 ? .patchworkGreen : .secondary, pulsing: runningCount > 0)
            ...
            Image(systemName: store.isCaffeinated ? "cup.and.saucer.fill" : "cup.and.saucer")
                .font(.system(size: PatchworkIcon.small, weight: .medium))
                .foregroundStyle(store.isCaffeinated ? Color.patchworkGreen : .secondary)
        ...
        .padding(.leading, PatchworkTheme.space12)
        .padding(.trailing, PatchworkTheme.space6)
...
struct SidebarSnapshot {
    ...
    private static func groups(
        ...
        func build(_ groupID: String, depth: Int) -> SessionFolderGroup? {
            let childIDs = depth >= PatchworkTheme.sidebarMaxFolderDepth ? [] : childrenByParent[groupID] ?? []
...
private struct StatusSectionHeader: View {
    ...
    var body: some View {
        Button(action: toggle) {
            ...
            .frame(height: PatchworkTheme.folderHeaderHeight)
            .contentShape(Rectangle())
...
private struct SessionFolderSection: View {
    ...
    private var indent: CGFloat { CGFloat(min(depth, PatchworkTheme.sidebarMaxFolderDepth)) * PatchworkTheme.sidebarIndentStep }
            ...
            Button {
                ...
                HStack(spacing: PatchworkTheme.space6) {
                    ...
                    Image(systemName: headerSymbol)
                        ...
                        .foregroundStyle(projectGit.isDirty ? Color.patchworkOrange : Color.secondary)
                        .frame(width: PatchworkTheme.sidebarIconColumn, alignment: .center)
                    Text(group.name)
                        .font(SidebarTypography.folderHeader).foregroundStyle(.secondary).lineLimit(1)
                    if hasRunning { StatusDot(color: .patchworkGreen, pulsing: true) }
                    Spacer(minLength: PatchworkTheme.space4)
                    newChatButton
                }
                .padding(.leading, PatchworkTheme.space8 + indent)
                .padding(.trailing, PatchworkTheme.space8)
                .frame(height: PatchworkTheme.folderHeaderHeight)
                .contentShape(Rectangle())
                .patchworkRowBackground(selected: false, hovering: hovering)
    ...
    private var newChatButton: some View {
        if hovering {
            Image(systemName: "plus")
                .font(.system(size: PatchworkIcon.small, weight: .medium))
                .foregroundStyle(.secondary)
                .frame(width: PatchworkTheme.sidebarIconColumn, height: PatchworkTheme.folderHeaderHeight, alignment: .center)
                .contentShape(Rectangle())
            ...
            Color.clear
                .frame(width: PatchworkTheme.sidebarIconColumn, height: PatchworkTheme.folderHeaderHeight)
...
private struct SessionRow: View {
    ...
    var body: some View {
        Button { store.selectSession(session) } label: {
            ...
            .padding(.leading, PatchworkTheme.sidebarIconInset + indent)
            .padding(.trailing, PatchworkTheme.space8)
            .frame(height: PatchworkTheme.sidebarRowHeight)
            .contentShape(Rectangle())
            .patchworkRowBackground(selected: selected, hovering: hovering)
    ...
    private var leadingIcon: some View {
        if hovering {
            Image(systemName: archived ? "tray.and.arrow.up" : "archivebox")
                .font(.system(size: PatchworkIcon.small, weight: .medium))
                .foregroundStyle(.secondary)
                .frame(width: PatchworkTheme.sidebarIconColumn, height: PatchworkTheme.sidebarRowHeight, alignment: .center)
                .contentShape(Rectangle())
            ...
            Color.clear
                .frame(width: PatchworkTheme.sidebarIconColumn, height: PatchworkTheme.sidebarRowHeight)
                .accessibilityHidden(true)
        }
    }
        ...
        HStack(spacing: PatchworkTheme.space6) {
            if running {
                ...
                Image(systemName: "arrow.triangle.branch")
                    .font(.system(size: PatchworkIcon.micro)).foregroundStyle(.tertiary)
