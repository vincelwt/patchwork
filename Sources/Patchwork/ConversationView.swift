            ...
            HStack(spacing: 0) {
                ...
                if showsInspector {
                    Rectangle()
                        ...
                        .frame(width: PatchworkTheme.hairline)
                    InspectorView(activities: store.runtimeActivities)
                        .transition(.move(edge: .trailing))
                }
            }
        }
        .patchworkPlainToolbar { conversationToolbar }
    ...
    private var conversationToolbar: some ToolbarContent {
        ...
        ToolbarItem(placement: .navigation) {
            HStack(spacing: PatchworkTheme.space6) {
                ...
                Menu {
                    ...
                    Image(systemName: "ellipsis").font(.system(size: PatchworkIcon.small, weight: .medium))
                }
                .menuStyle(.borderlessButton)
                .menuIndicator(.hidden)
                .frame(width: 16)
            }
            .frame(maxWidth: PatchworkTheme.conversationTitleMaxWidth, alignment: .leading)
            .contentShape(Rectangle())
        ...
        ToolbarItem(placement: .primaryAction) {
            Button { withAnimation(.easeOut(duration: 0.16)) { store.inspectorVisible.toggle() } } label: {
                Image(systemName: "sidebar.right").font(.system(size: PatchworkIcon.small, weight: .regular))
            }
...
struct MessageScrollView: View {
    ...
    var body: some View {
        ...
        ScrollView {
            ...
            LazyVStack(alignment: .leading, spacing: PatchworkTheme.transcriptEntrySpacing) {
                ...
                ForEach(items) { item in
                    Group {
                        ...
                        case let .message(message, isStreaming):
                            ...
                            .equatable()
                            .padding(.top, message.role == .user ? PatchworkTheme.transcriptTurnSpacing : 0)
                        case let .work(block):
                            TranscriptWorkView(
                                block: block,
                                onImage: store.showImage,
            ...
            .padding(.top, PatchworkTheme.space24)
            .padding(.bottom, PatchworkTheme.space8)
            .frame(maxWidth: PatchworkTheme.transcriptMaxWidth, alignment: .leading)
            .padding(.horizontal, PatchworkTheme.space20)
            .frame(maxWidth: .infinity)
            .background(
        ...
        .overlay(alignment: .bottom) {
            if !store.isBrowsingEarlierHistory, !isPinnedToBottom {
                ...
                .padding(.bottom, PatchworkTheme.space10)
                .transition(.opacity)
        ...
        ZStack {
            ...
            if isVisible {
                VStack(spacing: PatchworkTheme.space8) {
                    ProgressView().controlSize(.small)
                    Text("Opening conversation…").font(PatchworkFont.caption).foregroundStyle(.secondary)
                }
            }
        }
        .frame(maxWidth: .infinity, maxHeight: .infinity)
...
private struct EditingMessageBanner: View {
    ...
    var body: some View {
        ...
        .padding(.horizontal, PatchworkTheme.space10)
        .padding(.vertical, PatchworkTheme.space6)
        .background(Color.patchworkInset, in: RoundedRectangle(cornerRadius: PatchworkTheme.radiusMedium, style: .continuous))
...
private struct InlineError: View {
    ...
    var body: some View {
        HStack(spacing: PatchworkTheme.space8) {
            ...
            Text(text).font(PatchworkFont.caption).lineLimit(2)
            Spacer()
        ...
        .padding(.horizontal, PatchworkTheme.space10)
        .padding(.vertical, PatchworkTheme.space8)
        .background(Color.patchworkRed.opacity(0.08), in: RoundedRectangle(cornerRadius: PatchworkTheme.radiusMedium, style: .continuous))
    }
}
