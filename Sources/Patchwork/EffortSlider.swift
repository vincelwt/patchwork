...
/// own drawing: a capsule track that fills with `PatchworkTheme.effortRamp` up to the current step, and a
...
struct PatchworkEffortTrack: View {
    let modes: [AgentMode]
    let mode: AgentMode?
    ...
    private static var knobInset: CGFloat { PatchworkTheme.effortUltraKnobDiameter / 2 }
    ...
    private var index: Double {
        PatchworkEffortTrackGeometry.displayedIndex(dragIndex: dragIndex, pendingIndex: pendingIndex, reportedIndex: currentModeIndex)
    }
    ...
    private var knobDiameter: CGFloat { isHottest ? PatchworkTheme.effortUltraKnobDiameter : PatchworkTheme.effortKnobDiameter }
    private var knobTint: Color {
        mode == nil ? Color.secondary.opacity(0.4) : PatchworkTheme.effortColor(rank: currentModeIndex, of: steps)
    }

    var body: some View {
        GeometryReader { proxy in
            let width = proxy.size.width
            let knobX = PatchworkEffortTrackGeometry.knobCenterX(index: index, steps: steps, width: width, inset: Self.knobInset)

            ZStack(alignment: .leading) {
                Capsule()
                    .fill(Color.patchworkInset)
                    .frame(height: PatchworkTheme.effortTrackHeight)
                ...
                Capsule()
                    .fill(fillStyle)
                    .frame(width: knobX, height: PatchworkTheme.effortTrackHeight)

                Circle()
                    .fill(knobTint)
                    .overlay(Circle().stroke(Color.patchworkWindow, lineWidth: 1.5))
            ...
            .gesture(
                DragGesture(minimumDistance: 0)
                    .onChanged { value in
                        let raw = PatchworkEffortTrackGeometry.index(forX: value.location.x, steps: steps, width: width, inset: Self.knobInset)
                        dragIndex = raw
    ...
    private var fillStyle: LinearGradient {
        let stops = isHottest
            ? PatchworkTheme.effortRamp + [PatchworkTheme.effortUltraAccent]
            : PatchworkTheme.effortFill(rank: clampedIndex, of: steps)
        return LinearGradient(colors: stops, startPoint: .leading, endPoint: .trailing)
    }
}

/// Pure position math for `PatchworkEffortTrack`, kept separate from the view so the exact geometry bug
...
enum PatchworkEffortTrackGeometry {
