...
enum MarkdownInline {
    ...
    private static func parseAttributed(_ text: String, baseWeight: Font.Weight) -> AttributedString {
        ...
        result.font = .system(size: PatchworkFont.size, weight: baseWeight)
    ...
    private static func styleCodeSpans(
        ...
        for run in string.runs {
            ...
            string[run.range].font = .system(size: PatchworkFont.size, weight: weight, design: .monospaced)
            string[run.range].backgroundColor = .patchworkInset
        }
    }
...
private struct MarkdownBlockRow: View {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: PatchworkTheme.space4) {
                ForEach(Array(items.enumerated()), id: \.offset) { _, item in
                    HStack(alignment: .firstTextBaseline, spacing: PatchworkTheme.space6) {
                        Text(item.marker)
                            .font(ordered ? PatchworkFont.body.monospacedDigit() : PatchworkFont.body)
                            .foregroundStyle(.secondary)
                            .frame(minWidth: ordered ? 18 : 10, alignment: .trailing)
                        inline(item.text)
                    }
                    .padding(.leading, CGFloat(item.depth) * PatchworkTheme.space16)
                }
            }
        case let .quote(text):
        ...
        case .rule:
            PatchworkHairline().padding(.vertical, PatchworkTheme.space4)
        }
    }
    ...
    private func inline(_ text: String, baseWeight: Font.Weight = .regular) -> some View {
        Text(MarkdownInline.attributed(text, baseWeight: baseWeight))
            .lineSpacing(PatchworkFont.bodyLineSpacing)
...
struct MarkdownTableView: View {
    ...
    var body: some View {
        ScrollView(.horizontal, showsIndicators: false) {
            Grid(alignment: .topLeading, horizontalSpacing: PatchworkTheme.space16, verticalSpacing: PatchworkTheme.space10) {
                GridRow {
                ...
                Divider()
                    ...
                    .overlay(Color.patchworkHairline)
                    .padding(.vertical, PatchworkTheme.space2)
            ...
            .padding(.trailing, PatchworkTheme.space4)
        }
        .frame(maxWidth: .infinity, alignment: .leading)
