...
struct PatchworkApp: App {
    ...
    init() {
        ...
        CommandLineToolInstaller.migrateLegacyInstallation()
        for agent in AgentKind.allCases where AgentSkillInstaller.supports(agent) {
            _ = AgentSkillInstaller.migrateLegacyInstall(for: agent)
        }
...
struct RootView: View {
    ...
    @State private var currentWidth: CGFloat = PatchworkTheme.windowMinimumWidth
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            SidebarView()
                .navigationSplitViewColumnWidth(
                    min: PatchworkTheme.sidebarMinWidth,
                    ideal: PatchworkTheme.sidebarIdealWidth,
                    max: PatchworkTheme.sidebarMaxWidth
                )
                .toolbar(removing: .sidebarToggle)
                .patchworkPlainToolbar { sidebarToggleItem }
        } detail: {
            VStack(spacing: 0) {
                detail
        ...
        .onChange(of: columnVisibility) { _, visibility in
            if expectedPolicyVisibility == visibility {
                ...
                collapseState.userChangedVisibility(width: currentWidth, threshold: PatchworkTheme.sidebarAutoCollapseWidth)
            }
        }
        ...
        .overlay(alignment: .top) {
            if let toast = store.toast {
                ...
                .padding(.top, PatchworkTheme.space32 + PatchworkTheme.space24)
        ...
        .frame(minWidth: PatchworkTheme.windowMinimumWidth, minHeight: PatchworkTheme.windowMinimumHeight)
    ...
    private var sidebarToggleItem: some ToolbarContent {
        ToolbarItem(placement: .navigation) {
            Button {
                ...
                Image(systemName: "sidebar.left").font(.system(size: PatchworkIcon.small, weight: .regular))
            }
        ...
        if store.schedulesPresented {
            ...
            case .session:
                if store.selectedSession != nil { ConversationView() }
                    ...
                    PatchworkUnavailableView("Conversation not found", systemImage: "bubble.left")
    ...
    private func updateSidebar(for width: CGFloat) {
        ...
        guard let action = collapseState.action(
            ...
            threshold: PatchworkTheme.sidebarAutoCollapseWidth
        ) else { return }
