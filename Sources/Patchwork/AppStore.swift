...
struct DraftRecovery {
    ...
    static func restoredAttachments(sent: [ImageAttachment], current: [ImageAttachment]) -> [ImageAttachment] {
        ...
        return Array((sent.filter { !existing.contains($0.id) } + current).prefix(PatchworkTheme.imageCountLimit))
    }
}
...
final class RuntimeLeaseCoordinator {
    ...
    private static func isDaemonUnavailable(_ error: Error) -> Bool {
        if case PatchworkClientError.daemonUnreachable = error { return true }
        return false
    }

    private static func isThreadNotFound(_ error: Error) -> Bool {
        if case PatchworkClientError.notFound = error { return true }
        return false
    }
...
final class AppStore: ObservableObject {
    ...
    @Published var windowTitle = "Patchwork"
    ...
    private let daemonClient: PatchworkClient?
    ...
    private static let connectivityResumeCommand = "/patchwork-resume"
    ...
    private static let relaunchRecoveryInstruction =
        "Patchwork restarted during the previous turn. Inspect the durable conversation and continue from where it stopped. Do not repeat completed actions; if an outcome is uncertain, explain that and ask before repeating it."
    ...
    init(
        ...
        managedTurnResumer: @escaping ManagedTurnResumer = { path, instruction, clientID in
            let client = PatchworkKit.PatchworkClient.unixSocket(requestTimeout: 5)
            var ready = false
            var lastError: Error?
            ...
            _ = try await client.sendMessage(
                threadId: path,
                PatchworkKit.SendMessageRequest(text: instruction, clientId: clientID)
            )
        },
        managedTurnWriterProbe: ManagedTurnWriterProbe? = nil,
        daemonWorktreeProjectsURL: URL = PatchworkPaths.supportDirectory.appendingPathComponent("daemon-thread-overlay.json"),
        daemonThreadOverlayURL: URL? = nil,
        daemonClient: PatchworkClient? = nil,
    ...
    private func prepareInterruptedTurnContinuation(for slot: RuntimeSlot) {
        ...
        slot.runtime.send(type: "get_commands", payload: [:]) { [weak self, weak slot] result in
            ...
            let hasHelper = (response["data"]?["commands"]?.arrayValue ?? []).contains {
                $0["name"]?.stringValue == "patchwork-resume"
    ...
    var sidebarSnapshot: SidebarSnapshot {
        ...
        let snapshot = SidebarSnapshot(
            sessions: sessions,
            query: searchText,
            projectPaths: sidebarFolders.map(\.path),
    ...
    @discardableResult
    func consumeDaemonEvent(_ event: PatchworkEvent, reconciled: inout Bool) async -> Bool {
    ...
    private func refreshPullRequestStates(for summaries: [SessionSummary]) {
        ...
        pullRequestRefreshTask = Task { [weak self] in
            ...
            // Review polling belongs to Patchwork itself. It wakes the thread only after Codex
            ...
            for (session, url, deadline) in ready where !Task.isCancelled {
                ...
                do {
                    try await managedTurnResumer(
                        ...
                        "/patchwork-pr-review \(url.absoluteString) \(deadlineMilliseconds)",
    ...
    private func installActivityExtensionIfNeeded() {
        let isDisabled = ActivityExtensionSettings.isDisabled()
        Task.detached(priority: .utility) {
            let outcome = ActivityExtensionInstaller.run(isDisabled: isDisabled)
    ...
    func branchAndSubmitEditedMessage(
        ...
        let commandName = "patchwork-edit-message"
            ...
            let navigateAndSubmit: (String) -> Void = { [weak self, weak slot] entryID in
                ...
                slot.runtime.send(type: "get_commands", payload: [:]) { [weak self, weak slot] result in
                    ...
                    guard commandAvailable else {
                        ...
                        fail("Patchwork’s message editing helper is unavailable. Restart Patchwork and try again.", .warning)
                        return
                    }
                        ...
                        slot.runtime.send(type: "get_entries", payload: ["since": .string(entryID)]) { [weak self, weak slot] result in
                            ...
                            let marker = entries?.last(where: {
                                $0["type"]?.stringValue == "custom"
                                    && $0["customType"]?.stringValue == "patchwork-edit-ready"
    ...
    func admitAttachments(_ values: [ImageAttachment], existing: [ImageAttachment]) -> [ImageAttachment] {
        let valid = values.filter { $0.data.count <= PatchworkTheme.imageByteLimit }
        var accepted: [ImageAttachment] = []
        var bytes = existing.reduce(0) { $0 + $1.data.count }
        for value in valid where existing.count + accepted.count < PatchworkTheme.imageCountLimit {
            guard bytes + value.data.count <= PatchworkTheme.totalImageByteLimit else { continue }
    ...
    private func enforcingLoadedImageBudget(_ source: [ChatMessage]) -> [ChatMessage] {
        ...
        for messageIndex in result.indices.reversed() {
            ...
            for blockIndex in message.blocks.indices {
                ...
                if count < PatchworkTheme.imageCountLimit,
                   image.data.count <= PatchworkTheme.imageByteLimit,
                   bytes + image.data.count <= PatchworkTheme.totalImageByteLimit {
                    count += 1
    ...
    private func handleBackgroundRPCEvent(_ event: JSONValue, slot: RuntimeSlot) {
        ...
        case "agent_end", "summarization_retry_scheduled", "summarization_retry_attempt_start",
            ...
            break
        default:
            unknownRPCEvents.append(event.prettyPrinted(maxLength: PatchworkTheme.unknownPayloadLimit))
    ...
    private func handleBackgroundExtensionUI(_ event: JSONValue, slot: RuntimeSlot) {
        ...
        case "setTitle":
            slot.windowTitle = event["title"]?.stringValue?.suffixString(500) ?? "Patchwork"
    ...
    private func handleExtensionUI(_ event: JSONValue) {
        ...
        case "select", "confirm", "input", "editor":
            ...
            let request = ExtensionDialogRequest(
                ...
                raw: event.boundedFallback(maxLength: PatchworkTheme.unknownPayloadLimit)
            )
        ...
        case "setTitle": windowTitle = event["title"]?.stringValue ?? "Patchwork"
