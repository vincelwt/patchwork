...
extension Color {
    ...
    static func patchworkDynamic(light: NSColor, dark: NSColor) -> Color {
    ...
    static let patchworkTranscript = Color(nsColor: .textBackgroundColor)
    /// Window chrome behind panels.
    static let patchworkWindow = Color(nsColor: .windowBackgroundColor)
    /// Hairline separators.
    static let patchworkHairline = Color(nsColor: .separatorColor)

    /// Recessed neutral used for code, tool output, and inset detail. Never tinted.
    static let patchworkInset = patchworkDynamic(
    ...
    static let patchworkInsetStrong = patchworkDynamic(
        light: NSColor(white: 0.935, alpha: 1),
        dark: NSColor(white: 0.185, alpha: 1)
    )
    /// The user message bubble: neutral, not accent-tinted.
    static let patchworkUserBubble = patchworkDynamic(
        light: NSColor(white: 0.945, alpha: 1),
        dark: NSColor(white: 0.19, alpha: 1)
    )

    /// Neutral hover and selection for rows.
    static let patchworkHover = patchworkDynamic(
        light: NSColor(white: 0, alpha: 0.045),
        dark: NSColor(white: 1, alpha: 0.055)
    )
    static let patchworkSelection = patchworkDynamic(
        light: NSColor(white: 0, alpha: 0.075),
        dark: NSColor(white: 1, alpha: 0.10)
    )
    static let patchworkPressed = patchworkDynamic(
        light: NSColor(white: 0, alpha: 0.10),
        dark: NSColor(white: 1, alpha: 0.14)
    )

    // Semantic accents, used sparingly.
    static let patchworkGreen = Color(nsColor: .systemGreen)
    static let patchworkRed = Color(nsColor: .systemRed)
    static let patchworkOrange = Color(nsColor: .systemOrange)
    static let patchworkPurple = Color(nsColor: .systemPurple)
    static let patchworkBlue = Color(nsColor: .systemBlue)
}

// MARK: - Shared building blocks

/// A hairline separator from `separatorColor`, never a translucent gray band.
struct PatchworkHairline: View {
    var body: some View {
        Rectangle()
            .fill(Color.patchworkHairline)
            .frame(height: PatchworkTheme.hairline)
    }
}

/// A single recessed surface with no border and no shadow, so nothing ever doubles up.
struct PatchworkInsetModifier: ViewModifier {
    var radius: CGFloat = PatchworkTheme.radiusMedium
    var strong = false

    func body(content: Content) -> some View {
        content.background(
            strong ? Color.patchworkInsetStrong : Color.patchworkInset,
...
extension View {
    func patchworkInset(radius: CGFloat = PatchworkTheme.radiusMedium, strong: Bool = false) -> some View {
        modifier(PatchworkInsetModifier(radius: radius, strong: strong))
    }

    /// Restrained neutral hover/selection background used by every row in the app.
    func patchworkRowBackground(selected: Bool, hovering: Bool, radius: CGFloat = PatchworkTheme.radiusSmall) -> some View {
        background(
            selected ? Color.patchworkSelection : (hovering ? Color.patchworkHover : Color.clear),
            in: RoundedRectangle(cornerRadius: radius, style: .continuous)
        )
    }
    ...
    @ViewBuilder
    func patchworkPlainToolbar<C: ToolbarContent>(@ToolbarContentBuilder content: () -> C) -> some View {
...
struct PatchworkGridRow<Content: View>: View {
    let symbol: String
    ...
    var body: some View {
        HStack(alignment: .firstTextBaseline, spacing: PatchworkTheme.gridGutter) {
            Image(systemName: symbol)
                .font(.system(size: PatchworkIcon.small, weight: symbolWeight))
...
struct PatchworkGridProgressRow<Content: View>: View {
    @ViewBuilder var content: () -> Content
...
struct PatchworkUnavailableView<Actions: View>: View {
    let title: String
    ...
    var body: some View {
        VStack(spacing: PatchworkTheme.space8) {
            Image(systemName: systemImage)
                .font(.system(size: PatchworkIcon.large, weight: .regular))
                .foregroundStyle(.secondary)
            Text(title)
                .font(PatchworkFont.bodyEmphasis)
...
extension PatchworkUnavailableView where Actions == EmptyView {
...
extension View {
    ...
    @ViewBuilder
    func patchworkHardTopScrollEdge() -> some View {
        if #available(macOS 26.0, *) {
...
struct PatchworkSectionHeader: View {
    let title: String
    var trailing: String?
...
enum ConversationLayout {
    ...
    static var inspectorColumnWidth: CGFloat { PatchworkTheme.inspectorWidth + PatchworkTheme.inspectorGutter }
    ...
    static func showsInspector(requested: Bool, totalWidth: CGFloat) -> Bool {
        ...
        return totalWidth - inspectorColumnWidth >= PatchworkTheme.conversationMinimumWidth
    }
}
