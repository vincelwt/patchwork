...
struct SessionParser {
    ...
    static func summary(
        ...
        let preview = (firstPrompt?.headline(max: PatchworkTheme.sessionPreviewLimit)) ?? "No user message yet"
    ...
    static func chatMessage(fromAgentMessage message: JSONValue, id: String? = nil, budget: inout ImageBudget) -> ChatMessage? {
        ...
        return ChatMessage(
            ...
            raw: role == .unknown ? message.boundedFallback(maxLength: PatchworkTheme.unknownPayloadLimit) : .null
        )
    }
    ...
    private static func chatMessage(from entry: RawEntry, budget: inout ImageBudget) -> ChatMessage? {
        ...
        case "model_change", "thinking_level_change", "session_info", "label", "custom", "usage":
            return nil
        default:
            let fallback = entry.raw.boundedFallback(maxLength: PatchworkTheme.unknownPayloadLimit)
            return ChatMessage(
                id: entry.id,
                role: .unknown,
    ...
    static func contentBlocks(from content: JSONValue?, baseID: String, budget: inout ImageBudget) -> [MessageBlock] {
        ...
        for (index, raw) in (content.arrayValue ?? []).enumerated() {
            ...
            default:
                admitted.append(MessageBlock(id: id, kind: .unknown(type: type, raw: raw.boundedFallback(maxLength: PatchworkTheme.unknownPayloadLimit))))
            }
        }
        return admitted
    }
    ...
    private static func attachmentBlocks(_ attachments: [JSONValue], baseID: String, budget: inout ImageBudget) -> [MessageBlock] {
        ...
        for (index, raw) in attachments.prefix(PatchworkTheme.imageCountLimit).enumerated() {
    ...
    private static func imageBlock(
        ...
        guard budget.admitEncoded(length: encodedLength) else {
            let reason = encodedLength > PatchworkTheme.imageByteLimit * 2
        ...
        guard let data = Data(base64Encoded: encoded, options: [.ignoreUnknownCharacters]),
              data.count <= PatchworkTheme.imageByteLimit else {
