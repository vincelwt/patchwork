...
final class DecodedImageCache: @unchecked Sendable {
    ...
    private init() {
        values.countLimit = PatchworkTheme.decodedImageCountLimit
        values.totalCostLimit = PatchworkTheme.decodedImageByteLimit
    }
...
enum ImageThumbnailer {
    ...
    static var transcriptMaxPixel: CGFloat {
        2 * max(PatchworkTheme.transcriptImageMaxWidth, PatchworkTheme.transcriptImageMaxHeight)
    }
...
struct ImageAttachment: Identifiable, Hashable {
    static let temporaryDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent("Patchwork Images", isDirectory: true)
