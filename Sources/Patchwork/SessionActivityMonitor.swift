...
/// session, or one running under a Pi build without the `patchwork-activity` extension
/// installed yet.
...
///     session whose Pi process has the `patchwork-activity` extension loaded writes its own
...
final class SessionActivityMonitor: ObservableObject {
    ...
    /// `~/.pi/agent/patchwork-activity` directory and a real `kill(pid, 0)` liveness check.
    init(
