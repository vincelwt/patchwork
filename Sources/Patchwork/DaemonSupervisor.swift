...
extension PatchworkControlService: ControlServiceLifecycle {}

typealias ControlServiceFactory = @MainActor () async -> any ControlServiceLifecycle

/// Hosts the control plane inside Patchwork. The optional LaunchAgent/standalone daemon still
/// wins when explicitly installed, but the default path has no child `patchworkd` process: closing
...
final class DaemonSupervisor: ObservableObject {
    ...
    init(
        serviceFactory: @escaping ControlServiceFactory = {
            await Task.detached(priority: .utility) { PatchworkControlService() }.value
        },
        healthProbe: DaemonHealthProbing = SocketHealthProbe(),
        launchAgentProbe: LaunchAgentProbing = SystemLaunchAgentProbe(),
        ownerFileURL: URL = PatchworkPaths.supportDirectory.appendingPathComponent("daemon-owner.json"),
    ...
    /// One-release migration: the previous app may have crashed after spawning a standalone
    /// app-managed daemon. Retire only the PID in its ownership record, then reuse the same file
    /// for this app process so existing `patchwork daemon status` keeps reporting app-managed mode.
    private func retireLegacyAppManagedDaemon() async {
        ...
        // A live app-hosted record belongs to another Patchwork instance, not to the legacy
