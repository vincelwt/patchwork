...
final class ComposerImageAttachment: NSTextAttachment {
    ...
    init(attachment: ImageAttachment) {
        ...
        if let preview, preview.size.height > 0 {
            let scale = PatchworkTheme.inlineAttachmentHeight / preview.size.height
            let width = min(PatchworkTheme.inlineAttachmentMaxWidth, max(24, preview.size.width * scale))
            bounds = CGRect(x: 0, y: -3, width: width, height: PatchworkTheme.inlineAttachmentHeight)
        } else {
            bounds = CGRect(x: 0, y: -3, width: 40, height: PatchworkTheme.inlineAttachmentHeight)
        }
    }
...
struct NativeComposerTextView: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSScrollView {
        ...
        textView.font = PatchworkFont.composerNSFont
...
final class ComposerTextView: NSTextView {
    ...
    static var baseAttributes: [NSAttributedString.Key: Any] {
        [
            .font: PatchworkFont.composerNSFont,
...
enum ImageImportService {
    ...
    static func attachments(from pasteboard: NSPasteboard) -> [ImageAttachment] {
        ...
        if let data = pasteboard.data(forType: .png), data.count <= PatchworkTheme.imageByteLimit,
           let attachment = temporaryAttachment(data: data, mimeType: "image/png", fileName: "Pasted image.png") {
            return [attachment]
        }
        if let tiff = pasteboard.data(forType: .tiff), tiff.count <= PatchworkTheme.imageByteLimit,
           let image = NSImage(data: tiff), let png = pngData(from: image), png.count <= PatchworkTheme.imageByteLimit,
    ...
    static func attachments(from urls: [URL]) -> [ImageAttachment] {
        urls.prefix(PatchworkTheme.imageCountLimit).compactMap { url in
            guard let type = UTType(filenameExtension: url.pathExtension), type.conforms(to: .image),
                  let size = try? url.resourceValues(forKeys: [.fileSizeKey]).fileSize,
                  size <= PatchworkTheme.imageByteLimit,
    ...
    static func loadDroppedAttachments(
        ...
        }.prefix(PatchworkTheme.imageCountLimit))
    ...
    private static func loadDroppedImage(
        ...
        provider.loadDataRepresentation(forTypeIdentifier: identifier) { data, _ in
            guard let data, data.count <= PatchworkTheme.imageByteLimit,
                  let image = NSImage(data: data), let png = pngData(from: image),
                  png.count <= PatchworkTheme.imageByteLimit else { completion(nil); return }
    ...
    private static func providerReadyImage(
        ...
        guard max(width, height) > CGFloat(PatchworkTheme.imagePixelLimit) else {
        ...
        let options: [CFString: Any] = [
            ...
            kCGImageSourceThumbnailMaxPixelSize: PatchworkTheme.imagePixelLimit
        ]
        ...
        guard let encoded = NSBitmapImageRep(cgImage: image).representation(
            ...
            properties: encodingProperties
              ...
              encoded.count <= PatchworkTheme.imageByteLimit else { return nil }
