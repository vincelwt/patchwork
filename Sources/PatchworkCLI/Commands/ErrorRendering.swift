...
private func failure(for error: ControlPlaneError) -> CLIFailure {
    ...
    case let .unreachable(reason):
        return CLIFailure(
            ...
            message: "cannot reach the Patchwork daemon (\(reason))",
            hint: "start it with `patchwork daemon start` (or `patchwork daemon install` to run at login); check `patchwork daemon status`"
        )
