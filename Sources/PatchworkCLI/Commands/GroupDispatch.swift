...
/// Shared routing for `patchwork <group> <subcommand> [args]`. Every group command file builds a
...
func dispatchGroup(
    ...
    guard let handler = handlers[sub] else {
        context.out.errorLine("patchwork: unknown \(groupHelp.name) subcommand \"\(sub)\"")
