...
enum RemoteCommand {
    static let groupHelp = GroupHelp(
        name: "remote",
        usage: "patchwork remote <subcommand> [args]",
    ...
    private static let enableHelp = CommandHelp(
        usage: "patchwork remote enable [--port 7717] [--json]",
        ...
        examples: ["patchwork remote enable", "patchwork remote enable --port 7900"]
    )
    ...
    private static let urlHelp = CommandHelp(usage: "patchwork remote url [--json]", summary: "Print the loopback remote's URL.", flags: [], examples: ["patchwork remote url"])
    ...
    private static func url(_ args: [String], context: CommandContext) async -> Int32 {
        await runLeaf(args, context: context, help: urlHelp) { _, global in
            ...
            if global.jsonOutput {
                ...
                if !settings.enabled { context.out.info("(disabled — run `patchwork remote enable`)") }
            }
        }
    }

    private static let tokenHelp = CommandHelp(usage: "patchwork remote token [--json]", summary: "Print the bearer token for the loopback remote, generating one on first use.", flags: [], examples: ["patchwork remote token"])
    ...
    private static func report(context: CommandContext, global: GlobalOptions, enabled: Bool, port: Int, token: String?) {
        ...
        context.out.info("Restart the active host: reopen Patchwork, or run `patchwork daemon restart` for a LaunchAgent.")
    }
