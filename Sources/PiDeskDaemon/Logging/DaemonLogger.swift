...
/// Appends to `~/Library/Logs/Pi Desktop/daemon.log`, rotating a single backup so a daemon left
...
final class DaemonLogger: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "dev.pi.desktop.daemon.log")
    ...
    init(fileURL: URL = PiDeskPaths.logFile, maxBytes: Int = 8 * 1_024 * 1_024) {
