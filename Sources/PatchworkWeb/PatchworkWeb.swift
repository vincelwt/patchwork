...
///
/// `PatchworkWeb.asset(for:)` is the only entry point a request handler needs. Pass the request's
...
///
/// ```swift
/// if let asset = PatchworkWeb.asset(for: request.path) {
...
/// }
/// // path started with PatchworkWeb.apiPathPrefix ("/v1/") — route it to the control API instead.
/// ```
///
/// `asset(for:)` returns `nil` only for paths under `PatchworkWeb.apiPathPrefix`. The daemon's own
...
public enum PatchworkWeb {
    ...
    private static let siteRoot: URL = {
        let bundleName = "Patchwork_PatchworkWeb.bundle"
        ...
        fatalError("PatchworkWeb resource bundle is missing")
    }()
...
/// A small thread-safe memoizing cache. `PatchworkWeb.asset(for:)` may be called concurrently from
...
private final class AssetCache: @unchecked Sendable {
    private var storage: [String: PatchworkWeb.Asset] = [:]
    private let lock = NSLock()

    func get(_ key: String) -> PatchworkWeb.Asset? {
        lock.lock()
        defer { lock.unlock() }
        return storage[key]
    }

    func set(_ key: String, _ asset: PatchworkWeb.Asset) {
        lock.lock()
        defer { lock.unlock() }
        storage[key] = asset
    }
