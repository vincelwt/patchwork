...
/// `daemon.json`: the handful of settings `pidesk remote enable/disable` and the daemon itself
...
public struct DaemonSettings: Codable, Hashable, Sendable {
    ...
    public static func load(from url: URL = PiDeskPaths.daemonSettings) -> DaemonSettings {
        PiDeskFile.readIfPresent(DaemonSettings.self, from: url) ?? DaemonSettings()
    }

    public func save(to url: URL = PiDeskPaths.daemonSettings) throws {
        try PiDeskFile.writeAtomic(self, to: url)
    }
}
...
public enum DaemonToken {
    ...
    /// use this to get a token to enforce; callers on the client side (`pidesk remote token`)
    /// use it to display the token an operator pastes into a bearer header.
    public static func loadOrCreate(at url: URL = PiDeskPaths.tokenFile) throws -> String {
        ...
        let token = generate()
        try PiDeskFile.writeAtomic(Data(token.utf8), to: url)
        return token
    }
