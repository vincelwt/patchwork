...
public enum PiDeskJSON {
    public static let encoder: JSONEncoder = {
        let encoder = JSONEncoder()
        encoder.dateEncodingStrategy = .custom { date, encoder in
            var container = encoder.singleValueContainer()
            try container.encode(PiDeskDate.string(from: date))
        }
        return encoder
    }()
    ...
    public static let decoder: JSONDecoder = {
        ...
        decoder.dateDecodingStrategy = .custom { decoder in
            ...
            guard let date = PiDeskDate.date(from: raw) else {
...
public enum PiDeskFileError: Error, LocalizedError, Sendable {
    case notFound(URL)
...
/// `Application Support/Pi Desktop`: the directory `0700`, every file in it `0600`. Every
...
public enum PiDeskFile {
    ...
    public static func writeAtomic<T: Encodable>(_ value: T, to url: URL, encoder: JSONEncoder = PiDeskJSON.encoder) throws {
    ...
    public static func read<T: Decodable>(_ type: T.Type, from url: URL, decoder: JSONDecoder = PiDeskJSON.decoder) throws -> T {
        guard let data = FileManager.default.contents(atPath: url.path) else {
            throw PiDeskFileError.notFound(url)
        }
        do {
            return try decoder.decode(T.self, from: data)
        } catch {
            throw PiDeskFileError.decodingFailed(url, String(describing: error))
        }
    }

    public static func readIfPresent<T: Decodable>(_ type: T.Type, from url: URL, decoder: JSONDecoder = PiDeskJSON.decoder) -> T? {
