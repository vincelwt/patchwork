...
/// Teaches an agent that Pi Desktop's control CLI exists.
///
/// `pidesk` is already on every agent's PATH, so any of them could list threads, send messages,
/// and create automations — they simply have no way to know that. Pi learns it through the
/// extension Pi Desktop installs; Codex and Claude Code both read skills from a directory, so
...
public enum AgentSkillInstaller {
    public static let skillName = "pi-desktop"
    public static let versionMarkerPrefix = "<!-- pi-desktop-skill-version:"
    ...
    public enum Outcome: Equatable, Sendable {
        ...
        /// This agent has no skill directory Pi Desktop can write to.
        case unsupported
    ...
    ///
    /// Pi is deliberately absent: it already gets `pi-desktop-activity.ts`, which does strictly
    ...
    public static func source(for agent: AgentKind) -> String {
        ...
        ---
        name: \(skillName)
        description: Inspect and control Pi Desktop's conversations and automations through the `pidesk` CLI. Use when asked to list, read, or message Pi Desktop threads, or to create, inspect, pause, or remove scheduled automations for yourself or another agent.
        ---

        # Pi Desktop control

        `pidesk` talks to Pi Desktop's background service over a local socket. It is already on
        your PATH. Everything below is read-only unless it says otherwise.

        Run `pidesk --help`, or `pidesk <group> --help`, for the authoritative flag list; this
        file only covers what is easy to get wrong.

        ## Conversations

        ```
        pidesk threads list --agent \(agent.rawValue) --limit 20
        pidesk threads show <id> --messages 20
        pidesk threads send <id> "text"
        ```
        ...
        pidesk schedule add --name "Nightly triage" --cwd /path/to/project \\
            --prompt "…" --every 1h --agent \(agent.rawValue)
        pidesk schedule pause <id>
        pidesk schedule remove <id>
        ```
