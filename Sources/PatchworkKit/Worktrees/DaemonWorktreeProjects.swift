...
/// Read-only projection of the daemon-owned thread overlay. Patchwork merges this metadata into
...
public enum DaemonWorktreeProjects {
    ...
    public static func loadSnapshot(
        ...
        guard let data = boundedData(from: url),
              let payload = try? PatchworkJSON.decoder.decode(Payload.self, from: data) else {
            return Snapshot(
