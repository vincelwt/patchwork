...
enum HTTPWireFormat {
    ...
    static func parseResponse(_ raw: Data) throws -> RawHTTPResponse {
        guard let boundary = raw.range(of: Data("\r\n\r\n".utf8)) else {
            throw PatchworkClientError.invalidResponse("response had no header terminator")
        }
        guard let headerText = String(data: raw[..<boundary.lowerBound], encoding: .utf8) else {
            throw PatchworkClientError.invalidResponse("response headers were not valid UTF-8")
        }
        var lines = headerText.components(separatedBy: "\r\n")
        guard !lines.isEmpty else { throw PatchworkClientError.invalidResponse("response had no status line") }
        ...
        guard statusParts.count >= 2, let status = Int(statusParts[1]) else {
            throw PatchworkClientError.invalidResponse("could not parse status line \"\(statusLine)\"")
        }
        ...
        if let declared = headers["content-length"] {
            guard let length = Int(declared), length >= 0 else {
                throw PatchworkClientError.invalidResponse("response had an invalid Content-Length")
            }
            guard bodyBytes.count == length else {
                throw PatchworkClientError.invalidResponse(
