...
public enum PatchworkTransport: Sendable, Equatable {
    case unixSocket(path: String)
    case tcp(host: String, port: Int, token: String)
}

/// Every error `PatchworkClient` throws. `.daemonUnreachable` is deliberately its own case — every
...
public enum PatchworkClientError: Error, LocalizedError, Sendable {
    ...
    public var errorDescription: String? {
        switch self {
        case let .daemonUnreachable(detail): "Patchwork's daemon is not reachable: \(detail)"
...
public final class PatchworkClient: Sendable {
    public let transport: PatchworkTransport
    ...
    public init(transport: PatchworkTransport, requestTimeout: TimeInterval = 10, maxResponseBytes: Int = 16 * 1_024 * 1_024) {
        self.transport = transport
        self.requestTimeout = requestTimeout
        self.maxResponseBytes = maxResponseBytes
    }

    public static func unixSocket(at path: URL = PatchworkPaths.controlSocket, requestTimeout: TimeInterval = 10) -> PatchworkClient {
        PatchworkClient(transport: .unixSocket(path: path.path), requestTimeout: requestTimeout)
    }
    ...
    public func events() -> AsyncThrowingStream<PatchworkEvent, Error> {
                    ...
                    while !box.cancelled {
                        ...
                        if head == nil {
                            ...
                            guard pending.count <= maxBytes else {
                                throw PatchworkClientError.decodingFailed("GET /v1/events response header exceeded the client limit")
                            }
                            guard let split = HTTPWireFormat.splitHeadFromStream(pending) else { continue }
                            head = split.head
                            guard split.head.status == 200 else {
                                throw PatchworkClientError.server(status: split.head.status, code: "events_failed", message: "GET /v1/events returned \(split.head.status)")
                            }
                            for frame in parser.append(split.leftover) {
                                guard Self.yield(frame.decodedEvent(), to: continuation, box: box) else { return }
                            }
                            if parser.failed {
                                throw PatchworkClientError.decodingFailed("GET /v1/events contained an invalid or oversized frame")
                            }
                            continue
                        }
    ...
    private static func yield(
        _ event: PatchworkEvent,
        to continuation: AsyncThrowingStream<PatchworkEvent, Error>.Continuation,
        ...
        case .enqueued:
            return true
        case .dropped:
            continuation.finish(throwing: PatchworkClientError.daemonUnreachable("event consumer fell behind"))
            box.closeOnce()
            return false
        case .terminated:
            box.closeOnce()
    ...
    private func retryingProtectedMutation<Response>(
        ...
        guard await supports(capability) else {
            do {
                ...
                throw PatchworkClientError.outcomeUnknown(Self.outcomeUnknownMessage(for: error))
            }
        }
        var lastError: Error?
        ...
        throw lastError ?? PatchworkClientError.invalidResponse("protected mutation did not complete")
    }
    ...
    private static func outcomeUnknownMessage(for error: Error) -> String {
        guard let error = error as? PatchworkClientError else { return String(describing: error) }
        return switch error {
    ...
    private static func isRetryableCreationError(_ error: Error) -> Bool {
        guard let error = error as? PatchworkClientError else { return false }
        return switch error {
    ...
    private func send<Body: Encodable, Response: Decodable>(_ method: String, _ path: String, query: [String: String?] = [:], body: Body?) async throws -> Response {
        ...
        do {
            return try PatchworkJSON.decoder.decode(Response.self, from: data)
        } catch {
            throw PatchworkClientError.decodingFailed("\(method) \(path): \(error)")
        }
    }
    ...
    private func sendRaw<Body: Encodable>(_ method: String, _ path: String, query: [String: String?] = [:], body: Body?) async throws -> Data {
        ...
        let bodyData: Data? = try body.flatMap { try PatchworkJSON.encoder.encode($0) }
        ...
        return try await Self.performBlocking {
            ...
            } catch let error as PatchworkClientError {
                throw error
            } catch {
    ...
    private static func postConnectError(_ error: Error) -> PatchworkClientError {
        if let error = error as? PatchworkClientError { return error }
    ...
    private static func openConnection(transport: PatchworkTransport, connectTimeout: TimeInterval) throws -> Int32 {
        do {
            switch transport {
            ...
            throw PatchworkClientError.daemonUnreachable(error.localizedDescription)
        }
    }

    private static func baseHeaders(transport: PatchworkTransport) -> [String: String] {
        var headers = ["X-Patchwork-Api": "\(PatchworkAPI.apiVersion)"]
    ...
    private static func mapStatus(_ response: RawHTTPResponse) throws -> Data {
        ...
        case 401:
            throw PatchworkClientError.unauthorized
        case 404:
            let message = (try? PatchworkJSON.decoder.decode(APIErrorEnvelope.self, from: response.body))?.error.message ?? "Not found"
            throw PatchworkClientError.notFound(message)
        case 400...499:
            let envelope = try? PatchworkJSON.decoder.decode(APIErrorEnvelope.self, from: response.body)
            throw PatchworkClientError.badRequest(code: envelope?.error.code ?? "bad_request", message: envelope?.error.message ?? "Request rejected (\(response.status))")
        default:
            let envelope = try? PatchworkJSON.decoder.decode(APIErrorEnvelope.self, from: response.body)
            throw PatchworkClientError.server(status: response.status, code: envelope?.error.code ?? "server_error", message: envelope?.error.message ?? "Daemon error \(response.status)")
        }
    }
