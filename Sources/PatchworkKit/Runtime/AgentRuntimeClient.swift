...
public enum AgentRuntimeError: LocalizedError {
    ...
    public var errorDescription: String? {
        ...
        case let .notInstalled(agent):
            switch agent {
            case .pi: "Pi CLI was not found. Set PATCHWORK_PI_PATH or install pi in ~/.local/bin."
            case .codex: "Codex CLI was not found. Set PATCHWORK_CODEX_PATH or install codex."
            case .claude: "Claude Code was not found. Set PATCHWORK_CLAUDE_PATH or install claude."
            }
        case .notRunning:
...
public final class AgentRuntimeClient: AgentRuntimeProtocol {
    ...
    private let ioQueue = DispatchQueue(label: "app.patchwork.desktop.rpc", qos: .userInitiated)
    ...
    private let outputQueue = DispatchQueue(label: "app.patchwork.desktop.rpc.stdout", qos: .userInitiated, attributes: .concurrent)
    private let errorQueue = DispatchQueue(label: "app.patchwork.desktop.rpc.stderr", qos: .utility, attributes: .concurrent)
