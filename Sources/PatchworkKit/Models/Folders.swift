...
/// scheme (`Sources/Patchwork/WorkspaceOrganization.swift`): `nil` is top level, a filesystem
...
/// here because `Patchwork` is an executable target that `PatchworkKit` cannot import. The two must
...
public enum FolderTree {
    /// The deepest folder the app's own sidebar renders (`PatchworkTheme.sidebarMaxFolderDepth`): a
