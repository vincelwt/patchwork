...
final class SchedulesModelTests: XCTestCase {
    ...
    func testExplicitDaemonUnknownRunCodeRequiresReview() {
        let mapped = DaemonScheduleService.surfacedRun(PiDeskClientError.badRequest(
    ...
    func testRunIDConflictAlsoRequiresReview() {
        for source in [
            PiDeskClientError.badRequest(code: "run_id_conflict", message: "different work"),
            PiDeskClientError.server(status: 409, code: "run_id_conflict", message: "different work")
        ] {
