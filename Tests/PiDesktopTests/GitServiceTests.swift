...
final class GitServiceTests: XCTestCase {
    ...
    func testSnapshotAndCommandOutputStayBoundedInVeryDirtyRepository() async throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("PiDesktopGitBounded-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
        try runGit(["init", "-q"], at: root)
    ...
    private func makeScratchDirectories(_ names: [String]) throws -> (root: URL, urls: [URL]) {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("PiDesktopWorktreeFixture-\(UUID().uuidString)", isDirectory: true)
        var urls: [URL] = []
        for name in names {
    ...
    func testMainCheckoutReportsNoWorktreeInfo() async throws {
        let root = try makeTempRepo(name: "PiDesktopWorktreeMain")
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLinkedWorktreeReportsItsPathBranchAndMainCheckout() async throws {
        let root = try makeTempRepo(name: "PiDesktopWorktreeLinked")
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testDetachedLinkedWorktreeReportsANilBranch() async throws {
        let root = try makeTempRepo(name: "PiDesktopWorktreeDetached")
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testNestedSubdirectoryOfALinkedWorktreeStillResolvesToTheWorktreeRoot() async throws {
        let root = try makeTempRepo(name: "PiDesktopWorktreeNested")
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testNonRepositoryFolderSilentlyReportsNoWorktree() async throws {
        let root = FileManager.default.temporaryDirectory.appendingPathComponent("PiDesktopWorktreeNone-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
