...
final class AgentCommandParsingTests: XCTestCase {
    ...
    func testListAndDetailAreBoundedAndDeduplicated() {
        let overflow = (0..<(PiTheme.commandListLimit + 25)).map {
            command(["name": .string("cmd-\($0)"), "source": .string("skill")])
        }
        XCTAssertEqual(AgentCommand.parse(.array(overflow)).count, PiTheme.commandListLimit)
        ...
        XCTAssertEqual(long.first?.detail.count, PiTheme.commandDetailLimit)
    }
...
private struct CommandRepository: SessionRepositoryProtocol {
    var rootURL = URL(fileURLWithPath: "/tmp/pi-desktop-commands")
...
final class AppStoreCommandCacheTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PiDesktopCommands-\(UUID().uuidString)", isDirectory: true)
