    ...
    func testCommandOnlyReviewPollCompletesWithoutAgentSettled() async throws {
        ...
        while IFS= read -r line; do
          ...
          if [ "$count" -eq 1 ]; then
            ...
            printf '%s\\n' '{"type":"extension_ui_request","id":"done","method":"setStatus","statusKey":"patchwork-pr-review-complete"}'
        ...
        let outcome = await executor.execute(RunJob(
            ...
            prompt: "/patchwork-pr-review https://github.com/acme/widgets/pull/42 9999999999999",
