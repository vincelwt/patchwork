...
/// End-to-end: a real `HTTPServer` on a temp Unix socket, driven only through `PatchworkClient` \u2014
...
final class HTTPServerIntegrationTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        client = PatchworkClient(transport: .unixSocket(path: socketPath.path), requestTimeout: 5)
    }
    ...
    func testHealthReportsOkAndApiVersion() async throws {
        ...
        XCTAssertEqual(health.api, PatchworkAPI.apiVersion)
    ...
    func testPostConnectResponseLimitSurfacesAsTransportFailure() async {
        ...
        } catch PatchworkClientError.transportFailure {
    ...
    func testGetUnknownThreadReturns404() async throws {
        ...
        } catch PatchworkClientError.notFound {
            // expected
        }
    }
    ...
    func testRunningThreadBecomesUnreadOnlyAfterItFinishes() async throws {
        ...
        let writeHeartbeat: (String) throws -> Void = { state in
            ...
            try """
            {"sessionId":"sess-running","pid":\(getpid()),"state":"\(state)","updatedAt":"\(PatchworkDate.string(from: Date()))"\(completion)}
    ...
    func testPathlessHeartbeatDoesNotAttachToDuplicateSessionIDs() async throws {
        ...
        try """
        {"sessionId":"duplicate","pid":\(getpid()),"state":"running","updatedAt":"\(PatchworkDate.string(from: Date()))","completionId":"answer"}
        """.write(
    ...
    func testActivityClassifiesCopiedHistoriesByHeartbeatPath() async throws {
        ...
        let timestamp = PatchworkDate.string(from: Date())
    ...
    func testAmbiguousIDOnlyHeartbeatDoesNotHideOrAttachToACopiedHistory() async throws {
        ...
        let now = Date()
        let timestamp = PatchworkDate.string(from: now)
        try """
    ...
    func testActivityCollapsesMultipleLiveHeartbeatWritersForOneTranscript() async throws {
        ...
        let now = Date()
        let timestamp = PatchworkDate.string(from: now)
        let earlier = PatchworkDate.string(from: now.addingTimeInterval(-5))
        ...
        let oldest = PatchworkDate.string(from: now.addingTimeInterval(-8))
        try """
    ...
    func testPendingScheduleFreezesTargetAndAgentButAllowsAFullFormRename() async throws {
        ...
        } catch let PatchworkClientError.badRequest(code, message) {
    ...
    func testPairingClearlyReportsAnOfflineRelay() async throws {
        ...
        } catch let PatchworkClientError.server(status, code, _) {
