...
final class FakeThreadRPCService: ThreadRPCServing, @unchecked Sendable {
    ...
    init(thread: PatchworkThread, runtime: ThreadRuntimeState = ThreadRuntimeState()) {
        self.thread = thread
...
enum TestSupport {
    ...
    /// `~/Library/Application Support/Patchwork` or a real `~/.pi/agent/sessions`.
    static func makeCore(
        ...
        // A real `~/.pi/agent/patchwork-activity` can (and, on a machine with other agents active,
