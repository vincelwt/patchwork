...
final class MultiAgentThreadRoutingTests: XCTestCase {
    ...
    func testProtectedIdleCreatePublishesExactlyOnceForPiAndCodexReplays() async throws {
        ...
        let subscription = core.bus.subscribe { name, payload in
            if name == "thread", let thread = try? PatchworkJSON.decoder.decode(PatchworkThread.self, from: payload) {
...
private final class ThreadPublicationBox: @unchecked Sendable {
    ...
    private var values: [PatchworkThread] = []

    func append(_ value: PatchworkThread) {
        lock.lock()
        values.append(value)
        lock.unlock()
    }

    var all: [PatchworkThread] {
        lock.lock()
        defer { lock.unlock() }
        return values
    }
