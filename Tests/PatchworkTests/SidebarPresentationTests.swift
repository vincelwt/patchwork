    ...
    func testNewFolderActionLivesInTheSidebarContextMenu() throws {
        let source = try String(
            contentsOf: URL(fileURLWithPath: #filePath)
                ...
                .deletingLastPathComponent()
                .deletingLastPathComponent()
                .appendingPathComponent("Sources/Patchwork/SidebarView.swift"),
            encoding: .utf8
        )
...
final class SidebarTypographyTests: XCTestCase {
    func testConversationTitleUsesRowWeightBySelection() {
        XCTAssertEqual(SidebarTypography.conversationTitle(selected: false), PatchworkFont.row)
        XCTAssertEqual(SidebarTypography.conversationTitle(selected: true), PatchworkFont.rowEmphasis)
    }

    func testFolderHeaderConversationAndMetadataShareOnePointSize() {
        XCTAssertEqual(PatchworkFont.bodySize, PatchworkFont.size)
        XCTAssertEqual(PatchworkFont.metaSize, PatchworkFont.size)
    }
    ...
    func testEveryRoleResolvesToATypeScaleConstantNotAnAdHocLiteral() {
        ...
        XCTAssertEqual(SidebarTypography.folderHeader, PatchworkFont.captionEmphasis)
        XCTAssertEqual(SidebarTypography.metadata, PatchworkFont.micro)
        XCTAssertEqual(SidebarTypography.status, PatchworkFont.caption)
    }
}
