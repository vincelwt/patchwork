...
final class ConversationWorktreeTests: XCTestCase {
    ...
    private func message(_ text: String) -> ChatMessage {
        ChatMessage(
            id: UUID().uuidString,
            role: .assistant,
            blocks: [Patchwork.MessageBlock(id: UUID().uuidString, kind: .text(text))],
            timestamp: Date(),
            raw: .null
        )
    }
    ...
    func testOnlyPathsInsideTheWorktreeRootAreEverConsideredManaged() {
        XCTAssertTrue(WorktreeService.isManaged(WorktreeService.root.appendingPathComponent("patchwork-20260729-120000")))
        XCTAssertFalse(WorktreeService.isManaged(URL(fileURLWithPath: "/Users/someone/code/patchwork")))
    ...
    func testSlugCombinesRepositoryNameAndTimestampSoRepeatedWorktreesNeverCollide() {
        let first = WorktreeService.slug(forRepositoryNamed: "patchwork", now: Date(timeIntervalSince1970: 0))
        let second = WorktreeService.slug(forRepositoryNamed: "patchwork", now: Date(timeIntervalSince1970: 3_600))
        XCTAssertTrue(first.hasPrefix("patchwork-"))
    ...
    func testManagedWorktreeSessionStaysOrganizedUnderItsProject() throws {
        let project = "/Users/test/code/patchwork"
        let worktree = WorktreeService.root.appendingPathComponent("patchwork-20260729-120000").path
