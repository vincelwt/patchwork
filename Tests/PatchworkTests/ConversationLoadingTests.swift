...
final class ConversationLoadingTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        temporaryDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("PatchworkConversationLoading-\(UUID().uuidString)", isDirectory: true)
    ...
    func testEachHistoryPageKeepsABoundedImageBudget() async throws {
        ...
        store.selectSession(session)
        try await waitUntil { store.messages.count == 50 }
        XCTAssertEqual(store.messages.flatMap(\.images).count, PatchworkTheme.imageCountLimit)

        store.loadEarlierMessages()
        try await waitUntil { store.isBrowsingEarlierHistory && !store.isLoadingEarlierMessages }
        XCTAssertLessThanOrEqual(store.messages.flatMap(\.images).count, PatchworkTheme.imageCountLimit)
    ...
    func testLiveTailRefreshReprojectsFinishedSubagents() async throws {
        ...
        let launched = ChatMessage(
            id: "launch-result",
            role: .tool,
            blocks: [Patchwork.MessageBlock(id: "launch-result-text", kind: .text("Agent started in background."))],
        ...
        let stopped = ChatMessage(
            id: "wait-result",
            role: .tool,
            blocks: [Patchwork.MessageBlock(id: "wait-result-text", kind: .text("Agent stopped."))],
    ...
    func testLiveRefreshScanTruncationDoesNotFlagLoadedHistoryOutsideTheWindow() async throws {
        ...
        func message(_ id: String, role: Patchwork.MessageRole, text: String) -> ChatMessage {
            ChatMessage(id: id, role: role, blocks: [Patchwork.MessageBlock(id: "\(id)-text", kind: .text(text))], timestamp: nil, raw: .null)
        }
