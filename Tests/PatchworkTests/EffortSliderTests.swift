...
/// `PatchworkEffortTrack`'s bug was entirely in its position math, so it is pinned down here as plain
...
final class EffortSliderGeometryTests: XCTestCase {
    ...
    private let width = PatchworkTheme.effortTrackWidth
    private let inset = PatchworkTheme.effortUltraKnobDiameter / 2

    // MARK: - knobCenterX

    func testKnobCentreNeverHangsPastTheTrackEdges() {
        for index in 0..<steps {
            let center = PatchworkEffortTrackGeometry.knobCenterX(index: Double(index), steps: steps, width: width, inset: inset)
    ...
    func testKnobCentreAtEachStopIsEvenlySpacedAcrossTheInsetTrack() {
        let centers = (0..<steps).map { PatchworkEffortTrackGeometry.knobCenterX(index: Double($0), steps: steps, width: width, inset: inset) }
    ...
    func testKnobCentreClampsForOutOfRangeIndices() {
        XCTAssertEqual(
            PatchworkEffortTrackGeometry.knobCenterX(index: -5, steps: steps, width: width, inset: inset),
            inset,
            accuracy: 0.001
        )
        XCTAssertEqual(
            PatchworkEffortTrackGeometry.knobCenterX(index: 99, steps: steps, width: width, inset: inset),
            width - inset,
            accuracy: 0.001
        )
    }
    ...
    func testDegenerateWidthNeverDividesByZero() {
        XCTAssertEqual(PatchworkEffortTrackGeometry.knobCenterX(index: 2, steps: steps, width: 0, inset: inset), inset)
    }

    func testASingleStepFallsBackToTheMidpointInsteadOfDividingByZero() {
        XCTAssertEqual(PatchworkEffortTrackGeometry.knobCenterX(index: 2, steps: 1, width: width, inset: inset), width / 2)
    }
    ...
    func testIndexForXIsTheInverseOfKnobCentreX() {
        for tenth in 0...(steps - 1) * 10 {
            ...
            let x = PatchworkEffortTrackGeometry.knobCenterX(index: index, steps: steps, width: width, inset: inset)
            let resolved = PatchworkEffortTrackGeometry.index(forX: x, steps: steps, width: width, inset: inset)
    ...
    func testIndexForXClampsBeyondBothTrackEnds() {
        XCTAssertEqual(PatchworkEffortTrackGeometry.index(forX: -50, steps: steps, width: width, inset: inset), 0)
        XCTAssertEqual(PatchworkEffortTrackGeometry.index(forX: 5_000, steps: steps, width: width, inset: inset), Double(steps - 1))
    }

    func testIndexForXHandlesAZeroWidthTrack() {
        XCTAssertEqual(PatchworkEffortTrackGeometry.index(forX: 10, steps: steps, width: 0, inset: inset), 0)
    }

    // MARK: - displayedIndex (the drag-responsiveness fix)

    func testDisplayedIndexPrefersAnActiveDragOverEverythingElse() {
        XCTAssertEqual(PatchworkEffortTrackGeometry.displayedIndex(dragIndex: 1.5, pendingIndex: 3, reportedIndex: 0), 1.5)
    }
    ...
    func testDisplayedIndexHoldsThePendingReleaseUntilReconciled() {
        ...
        XCTAssertEqual(PatchworkEffortTrackGeometry.displayedIndex(dragIndex: nil, pendingIndex: 3, reportedIndex: 0), 3)
    }

    func testDisplayedIndexFallsBackToTheReportedModeOnceReconciled() {
        XCTAssertEqual(PatchworkEffortTrackGeometry.displayedIndex(dragIndex: nil, pendingIndex: nil, reportedIndex: 2), 2)
    }
}
