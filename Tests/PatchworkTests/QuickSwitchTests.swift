...
final class QuickSwitchScoringTests: XCTestCase {
    ...
    func testHundredsOfSessionsRankQuickly() {
        ...
        let started = Date()
        let ranked = QuickSwitchScoring.rank(sessions, query: "dashboards", limit: PatchworkTheme.quickSwitchResultLimit)
        let elapsed = Date().timeIntervalSince(started)

        XCTAssertEqual(ranked.count, PatchworkTheme.quickSwitchResultLimit)
