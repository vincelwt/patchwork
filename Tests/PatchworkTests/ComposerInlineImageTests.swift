...
final class ComposerInlineImageTests: XCTestCase {
    ...
    private func makeTextView() -> ComposerTextView {
        ...
        let representable = NativeComposerTextView(
            ...
            admitImages: { candidates, existing in
                Array(candidates.prefix(max(0, PatchworkTheme.imageCountLimit - existing.count)))
            },
            onHeightChange: { _ in }
        )
    ...
    func testOversizedImagesAreDownscaledForClaude() throws {
        ...
        XCTAssertEqual(representation.pixelsWide, PatchworkTheme.imagePixelLimit)
    ...
    func testPasteboardPNGBecomesAnAttachment() throws {
        ...
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("PatchworkTests-\(UUID().uuidString)"))
    ...
    func testInlineImagesPublishAComposerHeightAfterTheUpdatePass() async throws {
        ...
        XCTAssertGreaterThanOrEqual(published, PatchworkTheme.inlineAttachmentHeight)
        XCTAssertGreaterThan(published, PatchworkTheme.composerMinEditorHeight)
    }
    ...
    func testFinderDropInsertsInlineAttachments() throws {
        ...
        let pasteboard = NSPasteboard(name: NSPasteboard.Name("PatchworkDrop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testExistingAndNewConversationAreasRegisterImageFileDropTargets() throws {
        ...
        for file in ["ConversationView.swift", "NewChatView.swift"] {
            let source = try String(
                contentsOf: root.appendingPathComponent("Sources/Patchwork/\(file)"),
                encoding: .utf8
            )
    ...
    func testCountBudgetIsAppliedToInlineInsertion() throws {
        let textView = makeTextView()
        var urls: [URL] = []
        for index in 0..<(PatchworkTheme.imageCountLimit + 3) {
        ...
        XCTAssertEqual(attachments.count, PatchworkTheme.imageCountLimit)

        textView.insertImages(attachments, at: nil)
        XCTAssertEqual(textView.currentContent().attachments.count, PatchworkTheme.imageCountLimit)
