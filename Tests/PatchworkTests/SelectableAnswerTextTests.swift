...
final class SelectableAnswerTextTests: XCTestCase {
    ...
    func testAnAnswerWithATableStillMeasuresTallerThanASingleRow() {
        ...
        XCTAssertGreaterThan(view.height(forWidth: 600), PatchworkFont.size * 3, "Header plus two rows must all be laid out")
    }
    ...
    func testEverySettledAnswerProseBlockUsesSharedLineSpacing() throws {
        ...
        for fragment in ["paragraph", "heading", "list item", "quote"] {
            ...
            XCTAssertEqual(style.lineSpacing, PatchworkFont.bodyLineSpacing, accuracy: 0.001, fragment)
        }
    }
    ...
    func testHostedTextReportsItsWrappedHeightSynchronously() throws {
        ...
        XCTAssertGreaterThan(wide, PatchworkFont.size * 8, "Every block has to be accounted for, not just the first")
    ...
    func testMissingWidthProposalUsesARealFirstPassMeasure() {
        ...
        XCTAssertEqual(size.width, PatchworkTheme.transcriptMaxWidth)
        XCTAssertGreaterThan(size.height, PatchworkFont.size * 2, "A nil first proposal must not collapse the lazy row to one point")
    }
    ...
    func testSettledAnswerHostAllocatesHeightForTheWholeAnswer() throws {
        ...
        XCTAssertGreaterThan(textView.frame.height, PatchworkFont.size * 3, "The list must not be clipped below the first paragraph")
    }
    ...
    func testUserMessageUsesOneCompactSelectableRunAcrossParagraphs() throws {
        ...
        let availableTextWidth = 560 - PatchworkTheme.space32 * 2 - PatchworkTheme.space16 * 2
        XCTAssertLessThanOrEqual(narrowTextView.frame.width, availableTextWidth)
        XCTAssertGreaterThan(narrowTextView.frame.height, PatchworkFont.size * 3, "Long user messages must wrap")
    }
}
