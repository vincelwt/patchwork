...
final class CachedAccountProbeTests: XCTestCase {
    ...
    func testStatusBarDoesNotCapTheAccountLabelWidth() throws {
        ...
        let source = try String(contentsOf: root.appendingPathComponent("Sources/Patchwork/StatusBarView.swift"), encoding: .utf8)
        let start = try XCTUnwrap(source.range(of: "private struct CodexAccountControl"))
        let end = try XCTUnwrap(source.range(of: "// MARK: - fast mode", range: start.lowerBound..<source.endIndex))
