...
final class ImageBudgetTests: XCTestCase {
    ...
    func testSingleOversizedImageIsRejectedRegardlessOfRemainingBudget() {
        ...
        XCTAssertFalse(budget.admitEncoded(length: PatchworkTheme.imageByteLimit * 2 + 1))
    ...
    func testDefaultLimitsMatchDocumentedBudget() {
        ...
        XCTAssertEqual(PatchworkTheme.decodedImageCountLimit, 64)
        XCTAssertEqual(PatchworkTheme.decodedImageByteLimit, 64 * 1_024 * 1_024)
    }
}
...
final class ConversationLayoutTests: XCTestCase {
    func testInspectorColumnIsOnlyReservedWhenConversationKeepsUsableWidth() {
        ...
        XCTAssertEqual(column, PatchworkTheme.inspectorWidth + PatchworkTheme.inspectorGutter)

        // Exactly enough room for the conversation column.
        XCTAssertTrue(ConversationLayout.showsInspector(
            requested: true,
            totalWidth: column + PatchworkTheme.conversationMinimumWidth
        ))
        ...
        XCTAssertFalse(ConversationLayout.showsInspector(
            requested: true,
            totalWidth: column + PatchworkTheme.conversationMinimumWidth - 1
        ))
    ...
    func testDefaultWindowDetailWidthReservesTheInspector() {
        ...
        let detailWidth: CGFloat = 1_360 - PatchworkTheme.sidebarIdealWidth
