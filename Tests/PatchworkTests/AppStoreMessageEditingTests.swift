...
private struct FakeRepository: SessionRepositoryProtocol {
    var rootURL = URL(fileURLWithPath: "/tmp/patchwork-edit-tests")
...
final class AppStoreMessageEditingTests: XCTestCase {
    ...
    func testResubmitWhileIdleBranchesInTheSameSessionAndRewritesVisibleHistory() async throws {
        ...
        XCTAssertTrue(editCommand.hasPrefix("/patchwork-edit-message target-entry "))
    ...
    private func message(id: String, role: Patchwork.MessageRole, text: String) -> ChatMessage {
        ChatMessage(
            id: id, role: role,
            blocks: [Patchwork.MessageBlock(id: "\(id)-text", kind: .text(text))],
    ...
    private func editCommands() -> JSONValue {
        .object(["commands": .array([.object([
            "name": .string("patchwork-edit-message"), "source": .string("extension")
        ])])])
    }
    ...
    private func editMarker(targetID: String, token: String) -> JSONValue {
        .object([
            "entries": .array([.object([
                ...
                "customType": .string("patchwork-edit-ready"),
    ...
    private func completeEditNavigation(_ runtime: FakeRuntime, targetID: String) async throws {
        ...
        runtime.succeed("get_commands", data: editCommands())
        await waitUntil {
            runtime.lastPayload("prompt")?["message"]?.stringValue?.hasPrefix("/patchwork-edit-message ") == true
        }
        ...
        XCTAssertTrue(command.hasPrefix("/patchwork-edit-message \(targetID) "))
        runtime.succeed("prompt")
        await waitUntil { runtime.commandCount("get_entries") == 1 }
