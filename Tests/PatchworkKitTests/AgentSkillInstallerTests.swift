...
final class AgentSkillInstallerTests: XCTestCase {
    ...
    func testLegacyOwnedSkillMigratesAndAHandEditedSkillDoesNot() throws {
        let legacyFolder = root.appendingPathComponent("pi-desktop", isDirectory: true)
        try FileManager.default.createDirectory(at: legacyFolder, withIntermediateDirectories: true)
        let legacy = legacyFolder.appendingPathComponent("SKILL.md")
        try "<!-- pi-desktop-skill-version: 1 -->\n".write(to: legacy, atomically: true, encoding: .utf8)

        XCTAssertEqual(AgentSkillInstaller.migrateLegacyInstall(for: .claude, directory: root), .installed)
        XCTAssertTrue(FileManager.default.fileExists(atPath: destination.path))
        XCTAssertFalse(FileManager.default.fileExists(atPath: legacy.path))

        try FileManager.default.createDirectory(at: legacyFolder, withIntermediateDirectories: true)
        try "# My own skill\n".write(to: legacy, atomically: true, encoding: .utf8)
        XCTAssertNil(AgentSkillInstaller.migrateLegacyInstall(for: .claude, directory: root))
    ...
    func testAnOlderVersionIsUpgraded() throws {
        ...
        XCTAssertTrue(try String(contentsOf: destination, encoding: .utf8).contains("patchwork"))
    }
