...
final class AgentCatalogTests: XCTestCase {
    ...
    func testExecutableOverrideWinsOverSearchPath() throws {
        ...
        let resolved = AgentCatalog.executable(
            for: .codex,
            environment: ["PATCHWORK_CODEX_PATH": binary.path]
        )
    ...
    func testMissingExecutableResolvesToNil() {
        XCTAssertNil(AgentCatalog.executable(
            for: .claude,
            environment: ["PATCHWORK_CLAUDE_PATH": root.appendingPathComponent("absent").path]
        ))
    }
    ...
    func testInstalledListsOnlyResolvableAgentsInStableOrder() throws {
        ...
        let installed = AgentCatalog.installed(environment: [
            "PATCHWORK_PI_PATH": pi.path,
            "PATCHWORK_CLAUDE_PATH": claude.path,
            "PATCHWORK_CODEX_PATH": root.appendingPathComponent("absent").path
        ])
    ...
    func testSessionRootHonoursOverrideAndDefaultsUnderHome() {
        let override = AgentCatalog.sessionRoot(
            for: .codex,
            environment: ["PATCHWORK_CODEX_SESSION_DIR": root.path]
        )
    ...
    func testAgentForSessionPathMatchesTheOwningRoot() {
        let environment = [
            ...
            "PATCHWORK_CODEX_SESSION_DIR": root.appendingPathComponent("codex").path,
            "PATCHWORK_CLAUDE_SESSION_DIR": root.appendingPathComponent("claude").path
        ]
        XCTAssertEqual(
