...
/// never talks to a real pi-deskd.
final class RawHTTPClientTests: XCTestCase {
    ...
    func testProtectedSendPreservesUnreachableWhenNoListenerExists() async {
        let path = NSTemporaryDirectory() + "pidesk-protected-missing-\(UUID().uuidString).sock"
    ...
    func testUnreachableWhenNoListenerAtSocketPath() async {
        let path = NSTemporaryDirectory() + "pidesk-nonexistent-\(UUID().uuidString).sock"
