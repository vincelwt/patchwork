...
final class GlobalOptionsTests: XCTestCase {
    ...
    func testTokenFlagTakesPrecedenceOverEnv() throws {
        ...
        let options = try GlobalOptions.resolve(parsed, environment: ["PATCHWORK_TOKEN": "env-token"], commandOwnsTimeout: false)
    ...
    func testTimeoutEnvVarIsUsedWhenFlagAbsent() throws {
        let parsed = try parseArgs([], specs: GlobalFlag.all)
        let options = try GlobalOptions.resolve(parsed, environment: ["PATCHWORK_TIMEOUT": "45"], commandOwnsTimeout: false)
    ...
    /// as a request timeout, but $PATCHWORK_TIMEOUT remains a working escape hatch.
    func testCommandOwnedTimeoutIgnoresFlagButHonorsEnv() throws {
        ...
        let options = try GlobalOptions.resolve(parsed, environment: ["PATCHWORK_TIMEOUT": "20"], commandOwnsTimeout: true)
