...
export type Route =
  ...
  | { role: "client"; relayId: string; localPath: string };
...
export type HostFrame =
  | ProxyResponse
  | SocketReady
  | { type: "socket_data"; id: string; data: string }
...
export function parseHostFrame(value: unknown): HostFrame | null {
  ...
  if (value.type === "response") {
    ...
    if (!headerPairs(value.headers) || typeof value.body !== "string") return null;
  ...
  if (value.type === "socket_data") {
    return typeof value.data === "string" ? value as HostFrame : null;
  }
...
export function proxyHeaders(headers: Headers): [string, string][] {
  ...
  return [...headers].filter(([name]) => {
    ...
    return !blocked.has(lower) && !lower.startsWith("sec-websocket-");
  });
}
