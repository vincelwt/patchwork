...
- When a conversation opens a pull or merge request, the toolbar carries a quick link to it (`#482`), pointing at the most recent one. A successful `gh pr create` also starts a bounded five-minute heartbeat for that GitHub PR: pending review polls use only `gh api`, a clean Codex review stops silently, and review findings wake the same conversation to address, test, and push fixes without ever merging
