...
- **`SessionActivityMonitor` / `ActivityHeartbeatStore` / `ActivityExtensionInstaller`** — run and completion detection. Heartbeats carry the active branch's latest terminal assistant entry ID; a bounded tail fallback safely ignores torn writes. One monitor polls every 2 seconds while active and 15 seconds in the background, with at most 16 tail reads per tick.
