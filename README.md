...
- Sidebar rows carry their status on the trailing edge: a pulsing green dot while Pi is working, a blue dot when unread, and a clock when any automation (running or paused) targets that conversation. Running rows show live elapsed time and swap it for their Pi process tree's CPU and memory use on hover; the sidebar footer reports the whole Pi Desktop process tree plus any external running conversations, including managed subprocesses after their originating turn settles. Reduce Motion keeps working indicators static. The composer takes focus as soon as a conversation opens.
...
- **SwiftUI/AppKit views** — an AppKit-owned `NSScrollView` and exact-height `NSTableView` give transcript rows native scrolling, first-frame bottom pinning, and atomic stable-ID history prepends; existing SwiftUI message/work rows remain hosted inside it. Settled answer TextKit sizing is synchronous on the first layout pass. `NativeComposerTextView` supplies Return/Shift-Return and paste/drop semantics.
...
- Every conversation opens at the newest durable or live result. AppKit owns the transcript document and measures every inserted or changed row before one nonanimated table transaction: an exact tail pin follows streaming only until the user moves by one backing pixel, while an unpinned viewport restores the same durable row and glyph offset across history prepends. There are no delayed scroll corrections or estimated row heights; TextKit supplies settled-answer height synchronously. A loading indicator is delayed 120 ms so ordinary page reads do not flash.
