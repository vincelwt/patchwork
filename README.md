...
- Sidebar rows carry their status on the trailing edge: a green dot while Pi is working, a blue dot when unread, and a clock when any automation (running or paused) targets that conversation. Working indicators are static so background activity does not continuously redraw the transcript. The composer takes focus as soon as a conversation opens.
